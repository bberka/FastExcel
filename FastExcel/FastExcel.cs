...
namespace FastExcel
{
    ...
    public partial class FastExcel : IDisposable
  {
    ...
    public FastExcel(FileInfo excelFile,
                     bool readOnly = false) : this(null, excelFile, true, readOnly) { }
    ...
    public FastExcel(FileInfo templateFile,
                     FileInfo excelFile) : this(templateFile, excelFile, false) { }
    ...
    private FastExcel(FileInfo templateFile,
                      FileInfo excelFile,
                      bool updateExisting,
                      bool readOnly = false) {
      if (updateExisting) {
      ...
      TemplateFileStream = templateFile != null
                             ? new FileStream(templateFile.FullName, FileMode.Open, FileAccess.Read)
                             : null;
      ExcelFileStream = updateExisting
                          ? new FileStream(excelFile.FullName,
                                           FileMode.Open,
                                           readOnly
                                             ? FileAccess.Read
    ...
    /// <param name="updateExisting"></param>
    /// <param name="readOnly"></param>
    public FastExcel(Stream templateStream,
                     Stream excelStream,
                     bool updateExisting = false,
                     bool readOnly = false) {
      if (templateStream is FileStream templatefileStream) _templateFile = new FileInfo(templatefileStream.Name);
      if (excelStream is FileStream excelFileStream) _excelFile = new FileInfo(excelFileStream.Name);
    ...
    public FileInfo ExcelFile {
      get {
        if (_excelFile == null) throw new ApplicationException("ExcelFile was not provided");
        return _excelFile;
      }
    }
    ...
    public FileInfo TemplateFile {
      get {
        if (_templateFile == null) throw new ApplicationException("TemplateFile was not provided");
    ...
    /// </summary>
    private List<int> DeleteWorksheets { get; }

    /// <summary>
    ///   A list of worksheet indexs to insert
    /// </summary>
    private List<WorksheetAddSettings> AddWorksheets { get; }
    ...
    internal void PrepareArchive(bool openSharedStrings = true) {
      ...
      // Get Strings file
      if (SharedStrings == null && openSharedStrings) SharedStrings = new SharedStrings(Archive);
    }
    ...
    internal void CheckFiles() {
      if (_filesChecked) return;

      if (UpdateExisting) {
        if (ExcelFileStream?.Length == 0) throw new Exception("No input file name was supplied");
      }
      else {
        if (TemplateFileStream == null) throw new Exception("No Template file was supplied");
    ...
    private void UpdateRelations(bool ensureStrings) {
      ...
      using (var stream = Archive.GetEntry("xl/_rels/workbook.xml.rels").Open()) {
        ...
        var relationshipElements = document.Descendants().Where(d => d.Name.LocalName == "Relationship").ToList();
        var id = relationshipElements.Count;
        if (ensureStrings) {
          //Ensure SharedStrings
          var relationshipElement = (from element in relationshipElements
        ...
        if ((DeleteWorksheets != null && DeleteWorksheets.Any()) ||
            (AddWorksheets != null && AddWorksheets.Any())) {
          var worksheetElements = (from element in relationshipElements
                                   from attribute in element.Attributes()
          ...
          for (var i = worksheetElements.Length - 1; i > 0; i--) {
    ...
    private string[] UpdateWorkbook() {
      ...
      using (var stream = Archive.GetEntry("xl/workbook.xml").Open()) {
        ...
        if (document == null) throw new Exception("Unable to load workbook.xml");
        ...
        if (update) {
          ...
          var id = 1;
          foreach (var sheetElement in from sheet in document.Descendants()
                                       where sheet.Name.LocalName == "sheet"
                                       select sheet) {
    ...
    private void RenameAndRebildWorksheetProperties(XElement[] sheets) {
      ...
      var sheetProperties = (from sheet in sheets
      ...
      if (DeleteWorksheets != null && DeleteWorksheets.Any())
        foreach (var item in DeleteWorksheets) {
          var sheetToDelete = (from sp in sheetProperties
                               where sp.SheetId == item
                               select sp).FirstOrDefault();

          if (sheetToDelete != null) sheetProperties.Remove(sheetToDelete);
        }
      ...
      if (AddWorksheets != null && AddWorksheets.Any())
        ...
        foreach (var item in AddWorksheets.Reverse<WorksheetAddSettings>()) {
          var previousSheet = (from sp in sheetProperties
                               where sp.SheetId == item.InsertAfterSheetId
                               select sp).FirstOrDefault();

          if (previousSheet == null) throw new Exception(string.Format("Sheet name {0} cannot be added because the insertAfterSheetNumber or insertAfterSheetName is now invalid", item.Name));

          var newWorksheet = new WorksheetProperties {
      ...
      var index = 1;
      foreach (var worksheet in sheetProperties) {
        if (worksheet.CurrentIndex != index) {
          var entry = Archive.GetEntry(Worksheet.GetFileName(worksheet.CurrentIndex));
          if (entry == null)
            // TODO better message
    ...
    private void UpdateContentTypes(bool ensureStrings) {
      ...
      using (var stream = Archive.GetEntry("[Content_Types].xml").Open()) {
        ...
        var update = false;
        var overrideElements = document.Descendants().Where(d => d.Name.LocalName == "Override").ToList();

        //Ensure SharedStrings
        if (ensureStrings) {
          var overrideElement = (from element in overrideElements
                                 from attribute in element.Attributes()
        ...
        if (DeleteWorksheets != null && DeleteWorksheets.Any())
          foreach (var item in DeleteWorksheets) {
            ...
            var fileName = string.Format("/xl/worksheets/sheet{0}.xml", item);
        ...
        if (AddWorksheets != null && AddWorksheets.Any())
          foreach (var item in AddWorksheets) {
            // the file name is different for each xml file
            var fileName = string.Format("/xl/worksheets/sheet{0}.xml", item.SheetId);

            var overrideElement = new XElement(document.Root.GetDefaultNamespace() + "Override");

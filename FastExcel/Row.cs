...
namespace FastExcel
{
    ...
    public class Row
  {
      ...
      if (rowNumber <= 0) throw new Exception("Row numbers starting at 1");
      RowNumber = rowNumber;
      Cells = cells;
    }
      ...
      try {
        ...
        throw new Exception("Row Number not found in sheet: " + worksheet.Name, ex);
      }

      if (rowElement.HasElements) Cells = GetCells(rowElement, worksheet);
    }
    ...
    // </summary>
    private IEnumerable<Cell> GetCells(XElement rowElement,
                                       Worksheet worksheet) {
      foreach (var cellElement in rowElement.Elements()) yield return new Cell(cellElement, worksheet);
    }
    ...
    internal StringBuilder ToXmlString(SharedStrings sharedStrings) {
      ...
      if (Cells != null && Cells.Any()) {
        ...
        try {
          foreach (var cell in Cells) row.Append(cell.ToXmlString(sharedStrings, RowNumber));
        }
        finally {
          row.Append("</row>");
        }

...
namespace FastExcel
{
  public partial class FastExcel
  {
      ...
      public void Write<T>(IEnumerable<T> objectList,
                           int sheetNumber,
                           bool usePropertiesAsHeadings) {
      var worksheet = new Worksheet();
      worksheet.PopulateRows(objectList, 0, usePropertiesAsHeadings);
      Write(worksheet, sheetNumber, null);
    }
      ...
      Write(worksheet, null, sheetName);
    }
    ...
    private void Write(Worksheet worksheet,
      ...
      try {
        if (!UpdateExisting) TemplateFileStream.CopyTo(ExcelFileStream);
      }
      catch (Exception ex) {
      ...
      if (Archive.Mode != ZipArchiveMode.Update) throw new Exception("FastExcel is in ReadOnly mode so cannot perform a write");
      ...
      using (var stream = Archive.GetEntry(worksheet.FileName).Open()) {
        ...
        if (!worksheet.Template) worksheet.Headers = null;

        SharedStrings.ReadWriteMode = true;

        // Add Rows
        foreach (var row in worksheet.Rows) streamWriter.Write(row.ToXmlString(SharedStrings));
        SharedStrings.ReadWriteMode = false;

        //Add Footers
        streamWriter.Write(worksheet.Footers);
        if (!worksheet.Template) worksheet.Footers = null;
        streamWriter.Flush();

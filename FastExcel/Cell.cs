...
namespace FastExcel
{
    ...
    public class Cell
  {
      ...
      public Cell(int columnNumber,
                  object value) {
      if (columnNumber <= 0) throw new Exception("Column numbers starting at 1");
      ...
      public Cell(XElement cellElement,
                  Worksheet worksheet) {
      var isTextRow = (from a in cellElement.Attributes("t")
                       where a.Value == "s"
                       select a).Any();
      var columnName = (from a in cellElement.Attributes("r")
                        select a.Value).FirstOrDefault();

      var columnLetter = Regex.Replace(columnName, @"\d", "");
    ...
    internal StringBuilder ToXmlString(SharedStrings sharedStrings,
      ...
      if (Value != null) {
        ...
        if (Value is int)
          ...
          isString = false;
        else if (Value is string) isString = true;

        if (isString) value = sharedStrings.AddString(value.ToString());

        cell.AppendFormat("<c r=\"{0}{1}\"{2}>",
    ...
    public static int GetExcelColumnNumber(string columnName,
                                           bool includesRowNumber = true) {
      if (includesRowNumber) columnName = Regex.Replace(columnName, @"\d", "");

      var digits = new int[columnName.Length];
      for (var i = 0; i < columnName.Length; ++i) digits[i] = Convert.ToInt32(columnName[i]) - 64;
      var mul = 1;
      var res = 0;

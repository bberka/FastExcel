...
namespace FastExcel
{
    ...
    internal static class DefinedNamesExtensions
  {
      ...
      /// </returns>
      internal static List<string> FindCellNames(this IReadOnlyDictionary<string, DefinedName> definedNames,
                                                 string sheetName,
                                                 string columnLetter,
                                                 int rowNumber) {
      return (from e in definedNames where e.Value.Reference.Contains(sheetName + "!$" + columnLetter.ToUpper() + "$" + rowNumber) select e.Value.Name).ToList();
    }
      ...
      /// <returns></returns>
      internal static string FindColumnName(this IReadOnlyDictionary<string, DefinedName> definedNames,
                                            string sheetName,
                                            string columnLetter) {

...
namespace FastExcel
{
    ...
    public class CellRange
  {
    ...
    internal CellRange(string reference) {
      if (!Regex.IsMatch(reference, @"^[^\[\]\*\/\\\?\:]{1,31}\!\$[A-z]{1,4}:?\$")) throw new ArgumentException("CellRange reference argument is invalid or not supported.");

      var splitReference = reference.Split('!');

      SheetName = splitReference[0];
      var range = splitReference[1];
    ...
    public CellRange(string columnStart,
                     string columnEnd,
                     int rowStart = 1,
                     int? rowEnd = null) {

...
namespace FastExcel
{
    ...
    public class Worksheet
  {
    ...
    internal string FileName => GetFileName(Index);

    /// <summary>
    ///   Does the file exist
    /// </summary>
    public bool Exists => !string.IsNullOrEmpty(FileName);
    ...
    ///   Populate rows
    /// </summary>
    public void PopulateRows<T>(IEnumerable<T> rows,
                                int existingHeadingRows = 0,
                                bool usePropertiesAsHeadings = false) {
    ...
    /// </summary>
    public void PopulateRowsFromDataTable(DataTable table,
                                          int existingHeadingRows = 0) {
      ...
      for (var rowIndex = 0; rowIndex < table.Rows.Count; rowIndex++) {
        ...
        for (var columnIndex = 0; columnIndex < table.Columns.Count; columnIndex++) {
          var value = table.Rows[rowIndex].ItemArray[columnIndex];
    ...
    private string GetHeaderName(PropertyInfo propertyInfo) {
      ...
      if (descriptionAttribute != null && !string.IsNullOrWhiteSpace(descriptionAttribute.Name)) return descriptionAttribute.Name;
      return propertyInfo.Name;
    }

    private void PopulateRowsFromObjects<T>(IEnumerable<T> rows,
                                            int existingHeadingRows = 0,
                                            bool usePropertiesAsHeadings = false) {
      var rowNumber = existingHeadingRows + 1;
      ...
      foreach (var rowObject in rows) {
        ...
        foreach (var propertyInfo in properties) {
          var value = propertyInfo.GetValue(rowObject, null);
          if (value != null) {
    ...
    private void PopulateRowsFromIEnumerable(IEnumerable<IEnumerable<object>> rows,
                                             int existingHeadingRows = 0) {
      var rowNumber = existingHeadingRows + 1;

      var newRows = new List<Row>();

      foreach (var rowOfObjects in rows) {
        var cells = new List<Cell>();

        var columnNumber = 1;

        foreach (var value in rowOfObjects) {
          if (value != null) {
            var cell = new Cell(columnNumber, value);
            cells.Add(cell);
          }
    ...
    /// </summary>
    public void AddValue(int rowNumber,
                         int columnNumber,
                         object value) {
      if (Rows == null) Rows = new List<Row>();
    ...
    public void Merge(Worksheet data) {
      // Merge headings
      if (Headings == null || !Headings.Any()) Headings = data.Headings;
    ...
    internal void Read(int? sheetNumber = null,
    ...
    public void Read(int existingHeadingRows = 0) {
      ...
      using (var stream = FastExcel.Archive.GetEntry(FileName).Open()) {
    ...
    private IEnumerable<Row> GetRows(IEnumerable<XElement> rowElements) {
      foreach (var rowElement in rowElements) yield return new Row(rowElement, this);
    }
    ...
    internal void ReadHeadersAndFooters(StreamReader stream,
                                        ref Worksheet worksheet) {
      var headers = new StringBuilder();
      var footers = new StringBuilder();

      var headersComplete = false;
      var rowsComplete = false;

      var existingHeadingRows = worksheet.ExistingHeadingRows;
      ...
      while (stream.Peek() >= 0) {
        ...
        if (headersComplete && !rowsComplete) {
          if (existingHeadingRows == 0) rowsComplete = true;

          if (!rowsComplete)
            while (!string.IsNullOrEmpty(line) && existingHeadingRows != 0)
              if (line.Contains("<row")) {
                if (line.Contains("</row>")) {
                  var index = line.IndexOf("<row");
        ...
        if (rowsComplete) {
          if (line.Contains("</sheetData>")) {
            var index = line.IndexOf("</sheetData>") + "</sheetData>".Length;
    ...
    /// </summary>
    internal void GetWorksheetProperties(FastExcel fastExcel,
                                         int? sheetNumber = null,
                                         string sheetName = null) {
      GetWorksheetPropertiesAndValidateNewName(fastExcel, sheetNumber, sheetName);
    }

    private bool GetWorksheetPropertiesAndValidateNewName(FastExcel fastExcel,
                                                          int? sheetNumber = null,
                                                          string sheetName = null,
                                                          string newSheetName = null) {
      FastExcel = fastExcel;
      var newSheetNameExists = false;
      ...
      if (!sheetNumber.HasValue && string.IsNullOrEmpty(sheetName)) throw new Exception("No worksheet name or number was specified");

      using (var stream = FastExcel.Archive.GetEntry("xl/workbook.xml").Open()) {
        var document = XDocument.Load(stream);
        ...
        if (sheetNumber.HasValue) {
          ...
          if (sheetElement == null) throw new Exception(string.Format("There is no sheet named '{0}'", sheetName));
      ...
      if (!Exists) throw new Exception("No worksheet was found with the name or number was specified");
    ...
    internal void ValidateNewWorksheet(FastExcel fastExcel,
                                       int? insertAfterSheetNumber = null,
                                       string insertAfterSheetName = null) {
      ...
      var isNameValid = previousWorksheet.GetWorksheetPropertiesAndValidateNewName(fastExcel, insertAfterSheetNumber, insertAfterSheetName, Name);
      InsertAfterIndex = previousWorksheet.Index;

      if (!isNameValid) throw new Exception(string.Format("Worksheet name '{0}' already exists", Name));

      fastExcel.MaxSheetNumber += 1;
      Index = fastExcel.MaxSheetNumber;

      if (string.IsNullOrEmpty(Headers)) Headers = DEFAULT_HEADERS;

      if (string.IsNullOrEmpty(Footers)) Footers = DEFAULT_FOOTERS;
    }
  }
}
...
namespace FastExcel
{
  partial class FastExcel
  {
    ...
    internal IReadOnlyDictionary<string, DefinedName> DefinedNames {
      get {
        if (_definedNames == null) LoadDefinedNames();
    ...
    private void LoadDefinedNames() {
      ...
      if (document == null) throw new DefinedNameLoadException("Unable to load workbook.xml from open stream. Probable corrupt file.");
    ...
    public IEnumerable<IEnumerable<Cell>> GetCellRangesByDefinedName(string definedName,
                                                                     int? worksheetIndex = null) {
      var result = new List<List<Cell>>();

      var key = !worksheetIndex.HasValue
                  ? definedName
                  : definedName + ":" + worksheetIndex;

      if (!DefinedNames.ContainsKey(key)) return result;

      var references = DefinedNames[key].Reference.Split(',');
      ...
      foreach (var reference in references) {
        ...
        var worksheet = Read(cellRange.SheetName);
    ...
    /// <returns></returns>
    public IEnumerable<Cell> GetCellsByDefinedName(string definedName,
                                                   int? worksheetIndex = null) {
      var cells = new List<Cell>();
      var cellRanges = GetCellRangesByDefinedName(definedName) as List<List<Cell>>;

      foreach (var cellRange in cellRanges) cells.InsertRange(cells.Count, cellRange);
    ...
    /// <returns></returns>
    public IEnumerable<Cell> GetCellsByColumnName(string columnName,
                                                  int rowStart = 1,
                                                  int? rowEnd = null) {
      var columnCells = GetCellsByDefinedName(columnName) as List<Cell>;
      if (!rowEnd.HasValue) rowEnd = columnCells.Last().RowNumber;
